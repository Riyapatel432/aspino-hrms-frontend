...
export default function DashboardOverview() {
  const { can, isSuperAdmin, isEmployee, user } = usePermissions();
  ...
  const loadDashboardData = async (isManualRefresh = false) => {
    ...
    try {
      ...
      setStats({
        ...
        leavesPending: leavesList.filter((l) => l.status === STATUS.PENDING).length || 3,
  ...
  ].filter((card) => !card.subject || isSuperAdmin || can(card.action || "read", card.subject));
  ...
  const employeeKpiCards = [
    {
      ...
      trend: "Daily Clock",
      trendUp: true,
      desc: "Record Check-in & View Logs",
      icon: Clock,
      color: "from-blue-600 to-indigo-600",
      lightBg: "bg-blue-500/10 text-blue-600 dark:text-blue-400",
      link: "/dashboard/attendance-leave",
    ...
    },
    {
      ...
      value: "CL / SL / EL",
      trend: "Used & Remaining",
      trendUp: true,
      desc: "Remaining Leave Days Quota",
      icon: CalendarDays,
      color: "from-teal-500 to-emerald-600",
      lightBg: "bg-teal-500/10 text-teal-600 dark:text-teal-400",
      link: "/dashboard/attendance-leave",
  ...
  const renderedKpiCards = isEmployee
    ? employeeKpiCards
    : kpiCards;
  ...
  return (
    <div className="space-y-8 pb-12 animate-in fade-in duration-500">
          ...
          <div className="space-y-3 max-w-2xl">
            <div className="flex flex-wrap items-center gap-2.5">
              <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-extrabold tracking-wider uppercase bg-sky-500/20 text-sky-300 border border-sky-500/30">
                ...
                Aspino HRMS {isEmployee ? "Employee Portal" : "Portal"}
              </span>
            ...
            <h1 className="text-2xl sm:text-4xl font-extrabold tracking-tight text-white flex items-center gap-3">
              {isEmployee ? `Welcome, ${user?.name || "Employee"}` : "Employee Lifecycle Overview"}
            </h1>
            
            <p className="text-sm text-slate-300 leading-relaxed">
              {isEmployee
                ? "Manage your daily attendance, apply for leaves, check remaining vs used leave balances, and track your attendance matrix."
      ...
      {/* Employee Dashboard: 2-Column Layout with Time Utilization Card */}
      {isEmployee ? (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          {/* Left Column (8 cols): KPI Action Cards & Quick Self-Service */}
          <div className="lg:col-span-8 space-y-6">
            <section aria-label="Employee Key Portals" className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {employeeKpiCards.map((card, idx) => {
                const Icon = card.icon;
                return (
                  <Link
                    key={idx}
                    href={card.link}
                    className="group block relative bg-white dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800/80 rounded-3xl p-5 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 shadow-sm overflow-hidden focus:outline-none focus:ring-2 focus:ring-sky-500"
                  >
                    <div className="flex justify-between items-start">
                      <div className={`p-2.5 rounded-2xl ${card.lightBg}`}>
                        <Icon className="w-5 h-5" />
                      </div>
                      <span className="text-[11px] font-extrabold px-2 py-0.5 rounded-full bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-300">
                        {card.trend}
                      </span>
                    </div>
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-sm space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <Link
                  ...
                  className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-800 hover:border-sky-300 transition-all flex items-center gap-3 group"
                >
                  <div className="p-2.5 rounded-xl bg-sky-500/10 text-sky-600 dark:text-sky-400">
                    <Clock className="w-5 h-5" />
                  </div>
                  <div>
                    <span className="text-xs font-bold text-slate-900 dark:text-white block group-hover:text-sky-500">
                      Monthly Attendance Matrix
                    </span>
                    <span className="text-[11px] text-slate-400">View daily punch logs & presence credit</span>
                  </div>
                </Link>

                <Link
                  href="/dashboard/attendance-leave?tab=leaves"
                  className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-800 hover:border-amber-300 transition-all flex items-center gap-3 group"
                >
                  <div className="p-2.5 rounded-xl bg-amber-500/10 text-amber-600 dark:text-amber-400">
                    <CheckSquare className="w-5 h-5" />
                  </div>
                  <div>
                    <span className="text-xs font-bold text-slate-900 dark:text-white block group-hover:text-amber-500">
                      Apply Leave / Check Quota
                    </span>
                    <span className="text-[11px] text-slate-400">Casual, Sick & Earned leaves</span>
                  </div>
                </Link>
              </div>
            </div>
          ...
          {/* Right Column (4 cols): Dedicated Today's Time Utilization Widget */}
        ...
        /* Admin / HR KPI Cards */
        renderedKpiCards.length > 0 && (
          <section aria-label="Key HR Metrics" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {renderedKpiCards.map((card, idx) => {
              const Icon = card.icon;
              return (
                <Link
                  key={idx}
      ...
      {/* Visual Analytics Grid: Curved Area Chart & Interactive Department Cards (Admin/HR Only) */}
      {!isEmployee && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
      ...
      {/* Employee Self-Service Action Center (When isEmployee) */}
      {isEmployee && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {/* Card 1: Attendance Hub */}
          <div className="bg-white dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-md space-y-4 flex flex-col justify-between">
            <div className="space-y-3">
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
                Mark your daily check-in / check-out times, review your monthly attendance matrix, and track shift details.
              </p>
            </div>
            <Link
              href="/dashboard/attendance-leave"
              className="inline-flex items-center justify-center gap-2 w-full py-2.5 rounded-2xl bg-sky-500 hover:bg-sky-400 text-white font-bold text-xs shadow-md shadow-sky-500/20 transition-all"
            >
          ...
          <div className="bg-white dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-md space-y-4 flex flex-col justify-between">
            <div className="space-y-3">
              ...
              <h3 className="text-base font-bold text-slate-900 dark:text-white">
                Leave Requests & Remaining Quotas
              </h3>
              <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
                Apply for Casual, Sick, or Earned leaves. View your total allotted, used, and remaining leave balances for the fiscal year.
              </p>
            </div>
            <Link
              href="/dashboard/attendance-leave"
              className="inline-flex items-center justify-center gap-2 w-full py-2.5 rounded-2xl bg-amber-500 hover:bg-amber-400 text-white font-bold text-xs shadow-md shadow-amber-500/20 transition-all"
            >
      ...
      {/* Compliance & Action Center Grid (Admin/HR Only) */}
      {!isEmployee && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <section aria-label="Action Center" className="lg:col-span-6 bg-white dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-md space-y-4">
            ...
            <div className="space-y-3">
              {[
                ...
                .filter((t) => !t.subject || isSuperAdmin || can(t.action || "read", t.subject))
                .map((task, idx) => (
                  <div
                    key={idx}
      ...
      {/* Main HRMS Navigation Grid (Admin/HR Only) */}
      {!isEmployee && (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {[
              ...
              },
              {
                ...
                desc: "Access individual monthly salary slips, tax declarations, and attendance logs.",
                count: "Available",
                subject: "payroll",
                action: "read",
              },
            ]
              .filter((item) => !item.subject || isSuperAdmin || can(item.action || "read", item.subject))
              .map((item, idx) => {
      ...
      {/* Recent Activity Feed & Audit Trail (Visible if user has audit permission) */}
      {(isSuperAdmin || can("read", "audit")) && (
