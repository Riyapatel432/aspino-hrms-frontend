...
export default function LeaveMasterPage() {
  ...
  const canCreate = isSuperAdmin || can("create", "leave") || can("create", "leave_master") || can("create", "leave-master");
  const canUpdate = isSuperAdmin || can("update", "leave") || can("update", "leave_master") || can("update", "leave-master");
  const canDelete = isSuperAdmin || can("delete", "leave") || can("delete", "leave_master") || can("delete", "leave-master");
  const showForm = canCreate || (editingId && canUpdate);

  const startEditing = (row) => {
    if (!canUpdate) {
      toast.error("You do not have permission to edit leave masters");
      return;
    }
    setEditingId(row.id);
  ...
  const validateLeaveMaster = () => {
    const errs = {};
    if (!newLeaveMaster.department) errs.department = "Department is required.";
    if (!newLeaveMaster.fiscalYear) errs.fiscalYear = "Fiscal Year is required.";
  ...
  const handleSaveLeaveMaster = async (e) => {
    ...
    if (editingId ? !canUpdate : !canCreate) {
      toast.error(`You do not have permission to ${editingId ? "update" : "create"} leave masters.`);
      return;
    }
  ...
  const handleDeleteLeaveMaster = async () => {
    if (!deleteTarget || !canDelete) {
      toast.error("You do not have permission to delete leave masters");
      return;
    }
    setDeleting(true);
    try {
      ...
      console.error(err);
      toast.error(typeof err === "string" ? err : "Failed to delete Leave Master");
    } finally {
      setDeleting(false);
    }
  };
    ...
    },
    {
      ...
      render: (row) => (
        <div className="flex items-center gap-1.5">
          {(can("update", "leave") || can("update", "leave_master") || can("update", "leave-master")) && (
            <button
          ...
          {(can("delete", "leave") || can("delete", "leave_master") || can("delete", "leave-master")) && (
            <button
              onClick={() => {
  ...
  return (
    <RouteGuard subject="leave-master" action="read">
